...
def plot(MLIGHT,MZPRIME,color='dodgerblue'):
	# PEDRO's benchmark point
	alphaD 		= 0.25
	epsilon 	= np.sqrt(2.0*1.0e-10/const.alphaQED)
	UE4 		= 0.0
	# Umu4 		= np.sqrt(2.0e-8)#np.sqrt(1.0e-6)
	UMU4 		= np.sqrt(9.0e-7)
	UTAU4 		= 0.0
	...
	Tep = np.array([ fourvec.dot4(pep[i],fourvec.k0)-const.Me for i in range(size_samples)])
	Tem = np.array([ fourvec.dot4(pem[i],fourvec.k0)-const.Me for i in range(size_samples)])
...
plot(0.200,0.03,color='dodgerblue')
